...
struct monoid
{
	...
	constexpr static long long mod = 998244353;
	static pll query_op(pll a, pll b)
	{
		return (std::make_pair((a.first * b.first) % mod, (a.second * b.first + b.second) % mod));
	}
	...
	static pll update_op(pll a, pll b)
	{
		return (b);
	}

	static pll update_id()
	{
		return (std::make_pair(2e18, 2e18));
	}

	static pll apply(pll a, pll b, int len)
	{
		return (b == update_id() ? a : b);
	}
};
...
int main()
{
	...
	toolbox::datastructure::implicit_treap<std::pair<long long, long long>, monoid> treap(vec);
	while (q--)
	{
		int op;
		std::cin >> op;
		if (op == 0)
		{
			long long p, c, d;
			std::cin >> p >> c >> d;
			treap.update(p, p + 1, std::make_pair(c, d));
		}
		else
		{
			int l, r, x;
			std::cin >> l >> r >> x;
			auto res = treap.query(l, r);
			std::cout << (res.first * x + res.second) % monoid::mod << std::endl;
		}
	}
	return (0);
}